...
//
//ArkShaderReflectionFactory11.cpp
//--------------------------------------------------------------------------------
#include "Pch.h"
...
//--------------------------------------------------------------------------------
ArkShaderReflectionFactory11::ArkShaderReflectionFactory11( )
{
}
//--------------------------------------------------------------------------------
ArkShaderReflectionFactory11::~ArkShaderReflectionFactory11()
...
//--------------------------------------------------------------------------------
ArkShaderReflection11* ArkShaderReflectionFactory11::GenerateReflection( ArkShader11& shader )
{
	...
	IParameterManager* pParamMgr = ArkRenderer11::Get()->m_pParamMgr;
	...
	ArkShaderReflection11* pReflection = new ArkShaderReflection11();
	...
	if ( FAILED( hr ) )
	{
		ArkLog::Get(LogType::Renderer).Output( L"Failed to create shader reflection interface!" );
		return( nullptr );
	}
	...
	for ( UINT i = 0; i < desc.ConstantBuffers; i++ )
	{
		...
		if ( bufferDesc.Type == D3D_CT_CBUFFER || bufferDesc.Type == D3D_CT_TBUFFER )
		{
			...
			BufferLayout.pParamRef = pParamMgr->GetConstantBufferParameterRef( 
				std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t>().from_bytes( std::string( BufferLayout.Description.Name ) ) );
			...
			for ( UINT j = 0; j < BufferLayout.Description.Variables; j++ )
			{
							...
							( type_desc.Class == D3D_SVC_MATRIX_COLUMNS ) )
				{
					...
					if ( count == 0 ) 
					{
						pParam = pParamMgr->GetMatrixParameterRef( std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t>().from_bytes( std::string( var_desc.Name ) ) );
					}
					else
					{
