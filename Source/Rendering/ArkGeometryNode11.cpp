...
//--------------------------------------------------------------------------------
ArkGeometryNode11::ArkGeometryNode11()
...
//--------------------------------------------------------------------------------
void ArkGeometryNode11::Execute(PipelineManager* pPipeline,IParameterManager* pParamManager)
{
	for(int i=0; i < Children.size(); i++)
	{
		Children[i].Execute(pPipeline,pParamManager);
	}
...
//--------------------------------------------------------------------------------
void ArkGeometryNode11::AddElement(VertexElement11* element)
{
	int index = -1;
...
////--------------------------------------------------------------------------------
void ArkGeometryNode11::AddFace(TriangleIndices& face)
{
...
////--------------------------------------------------------------------------------
void ArkGeometryNode11::AddLine(LineIndices& line)
{
	m_vIndices.push_back(line.P1());
	m_vIndices.push_back(line.P2());
}
//--------------------------------------------------------------------------------
void ArkGeometryNode11::AddPoint(PointIndices& point)
{
	m_vIndices.push_back(point.P1());
}
//--------------------------------------------------------------------------------
void ArkGeometryNode11::AddIndex(UINT index)
{
	m_vIndices.push_back(index);
}
//--------------------------------------------------------------------------------
VertexElement11* ArkGeometryNode11::GetElement(std::string name)
{
...
//--------------------------------------------------------------------------------
VertexElement11* ArkGeometryNode11::GetElement(std::wstring name)
{
	std::string asciiName = std::wstring_convert<std::codecvt_utf8<wchar_t>,wchar_t>().to_bytes(name);
	return GetElement(asciiName);
}
//--------------------------------------------------------------------------------
VertexElement11* ArkGeometryNode11::GetElement(int index)
{
	return(m_vElements[index]);
}
//--------------------------------------------------------------------------------
UINT ArkGeometryNode11::GetIndex(unsigned int index)
{
...
//--------------------------------------------------------------------------------
D3D11_PRIMITIVE_TOPOLOGY ArkGeometryNode11::GetPrimitiveType()
{
	return(m_ePrimType);
}
//--------------------------------------------------------------------------------
int ArkGeometryNode11::GetPrimitiveCount()
{
	UINT count = 0;
...
//--------------------------------------------------------------------------------
void ArkGeometryNode11::SetPrimitiveType(D3D11_PRIMITIVE_TOPOLOGY type)
{
	for(int i=0; i < Children.size(); i++)
	{
		Children[i].SetPrimitiveType(type);
	}
	m_ePrimType = type;
}
//--------------------------------------------------------------------------------
int ArkGeometryNode11::GetVertexCount()
{
	return(m_iVertexCount);
}
//--------------------------------------------------------------------------------
int ArkGeometryNode11::GetElementCount()
{
	return(m_vElements.size());
}
//--------------------------------------------------------------------------------
int ArkGeometryNode11::GetVertexSize()
{
	return(m_iVertexSize);
}
//--------------------------------------------------------------------------------
int ArkGeometryNode11::CalculateVertexSize()
{
...
//--------------------------------------------------------------------------------
int ArkGeometryNode11::CalculateVertexCount()
{
...
//--------------------------------------------------------------------------------
void ArkGeometryNode11::GenerateInputLayout(int ShaderID)
{
	if(NodeProps->NodeName == L"ArkSceneRoot")
	{
		Children[0].GenerateInputLayout(ShaderID);
		for( int i=0; i < Children.size(); i++ )
		{
			Children[i].m_InputLayouts = Children[0].m_InputLayouts;
		}
	}
	else
	{
...
//--------------------------------------------------------------------------------
void ArkGeometryNode11::LoadToBuffers()
{
	for(int i=0; i < Children.size(); i++)
	{
		Children[i].LoadToBuffers();
	}
	// Check the number of vertices to be created
	CalculateVertexCount();
	...
	if(GetVertexSize() > 0)
	{
		...
		data2.pSysMem = reinterpret_cast<void*>(&m_vIndices[0]);
		data2.SysMemPitch = 0;
		data2.SysMemSlicePitch = 0;

		ArkBuffer11Config ibuffer;
		ibuffer.SetDefaultIndexBuffer(sizeof(UINT) * GetIndexCount(),false);
		m_IB = ArkRenderer11::Get()->CreateIndexBuffer(&ibuffer,&data2);
	}
}
//--------------------------------------------------------------------------------
UINT ArkGeometryNode11::GetIndexCount()
{
	return(m_vIndices.size());
}
//--------------------------------------------------------------------------------
//--------------------------------------------------------------------------------
bool ArkGeometryNode11::ComputeTangentFrame(std::string positionSemantic,
	std::string normalSemantic,
	std::string texCoordSemantic,
	std::string tangentSemantic)
{
	for(int i=0; i < Children.size(); i++)
		Children[i].ComputeTangentFrame(positionSemantic,normalSemantic,texCoordSemantic,tangentSemantic);
