...
//--------------------------------------------------------------------------------
ArkShaderReflection11::ArkShaderReflection11()
{
}
//--------------------------------------------------------------------------------
ArkShaderReflection11::~ArkShaderReflection11()
...
//--------------------------------------------------------------------------------
void ArkShaderReflection11::InitializeConstantBuffers( IParameterManager* pParamManager )
{
		...
		if ( ConstantBuffers[i].Description.Type == D3D11_CT_CBUFFER )
		{
			...
			if ( index == -1 )
			{
				...
				for ( int thread = 0; thread < NUM_THREADS + 1; thread++ )
				{
					...
					ResourcePtr resource = ArkRenderer11::Get()->CreateConstantBuffer( &cbuffer, 0 );
...
//--------------------------------------------------------------------------------
void ArkShaderReflection11::UpdateParameters( PipelineManager* pPipeline, IParameterManager* pParamManager )
{
	...
	for ( unsigned int i = 0; i < ConstantBuffers.size(); i++ )
	{
		if ( ConstantBuffers[i].Description.Type == D3D11_CT_CBUFFER )
		{
			...
			if ( index == -1 )
			{
				...
				ArkLog::Get(LogType::Renderer).Output( L"Uh oh - creating a constant buffer in the ShaderDX11::UpdateParameters functions!!!!" );
...
//--------------------------------------------------------------------------------
void ArkShaderReflection11::BindParameters( ShaderType type, PipelineManager* pPipeline, IParameterManager* pParamManager )
{
...
//--------------------------------------------------------------------------------
void ArkShaderReflection11::PrintShaderDetails()
{
	std::wstringstream s;
...
/*			s << L", Class: " << TO_STRING_D3D_SHADER_VARIABLE_CLASS( ConstantBuffers[i].Types[j].Class );
	...
	ArkLog::Get( LogType::Renderer).Output( s.str() );
}
//--------------------------------------------------------------------------------
void ArkShaderReflection11::SetName( const std::wstring& name )
{
	Name = name;
}
//--------------------------------------------------------------------------------
std::wstring ArkShaderReflection11::GetName()
{
	return Name;
}
//--------------------------------------------------------------------------------
