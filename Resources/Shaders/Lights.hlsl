...
float4 PositionFromLight(in float4 position)
{
	float4x4 WVP =
		mul(mul(gWorld,LightView),LightProj);
...
PSOutput PSMain(in VSOutput input,in float4 screenPos : SV_Position,in uint coverage : SV_Coverage)
{
	...
	GetGBufferAttributes(screenPos.xy,input.ViewRay,0,normal,position,diffuseAlbedo,
		specularAlbedo,specularPower);
	lighting = CalcLighting(normal,position,diffuseAlbedo,specularAlbedo,specularPower);
	float4 lightingPosition = PositionFromLight(float4(position,1.0f));

	float2 ShadowTexC = 0.5f * lightingPosition.xy / lightingPosition.w + float2(0.5,0.5);
		ShadowTexC.y = 1.0f - ShadowTexC.y;
	float shadowDepth = ShadowMap.Sample(ShadowMapSampler,ShadowTexC).r;



	float depth = 1 - (lightingPosition.z / lightingPosition.w);

	if(shadowDepth-0.03f > depth)
	{
